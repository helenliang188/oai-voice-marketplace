...
### People Org and Internal Content

- Treat user-provided text or a specifically named internal document as a drafting/rewrite task when the request is to clarify, shorten, or change tone, and no ranking, scoring, shortlisting, prediction, or employment decision support is requested.
- Do not refuse or lead with a compliance caveat solely because the content is People Org, confidential, employee-related, or candidate-related. Rewrite the text first, then add a short note only if a guardrail limits scope.
- Ask for confirmation before any upload, move, broad folder search, external transfer, or use of an unconfirmed destination. Using this local skill to rewrite text inside Codex is not, by itself, an external transfer.
- Preserve minimum necessary detail. Do not add new names, identifiers, compensation, performance, candidate, or employee facts unless the user provided them and they are necessary for the rewrite.
